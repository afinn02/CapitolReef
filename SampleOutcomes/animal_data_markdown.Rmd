---
title: "Capitol Reef Wildlife Monitoring"
...
<br> NOTE: Repeats are standardized by removing the columns where repeat is set to yes. The counts of species was aggregated using the Count_Species1 column which has been modified to include both Species1 & Species2 sightings.
...
# Now plot the data
p0 <- ggplot(daily_sightings, aes(x = .data$Date, y = Sightings)) +  # Set the line color to green
...
p <- plot_ly(labels = labels, values = counts, type = 'pie', 
             textinfo = 'label+percent', hoverinfo = 'label+percent+value',
             marker = list(colors = RColorBrewer::brewer.pal(length(counts), "Paired")))

# Set layout options
p <- p %>% layout(title = "Overall Percentage of Bighorn Sheep Detection by Sex")
...
p <- ggplot(plot_df, aes(x = factor(Year), y = percent, fill = Sex_Species1)) +
  ...
  labs(title = "Annual Percentage of Bighorn Sheep by Year and Gender (Park-wide)",
       x = "Year", 
       y = "Percentage") +
...
#### Percentage of BHS by Region, Year, Month, & Sex Relative to Region Totals
...
for(region in unique_regions) {
  # Ensure region is a character string
  region <- as.character(region)
  
  # Filter the data for the current region and years with data
  region_data <- filter(monthly_complete, Region == region)
  
  # Determine the years that actually have data
  years_with_data <- region_data %>%
    group_by(Year) %>%
    summarise(Total = sum(Percentage, na.rm = TRUE)) %>%
    filter(Total > 0) %>%
    .$Year
  
  # Skip plotting for this region if there are no years with data
  if(length(years_with_data) == 0) next
  
  # Filter the region_data for only years with data
  region_data <- region_data %>%
    filter(Year %in% years_with_data)
  
  # Determine the number of columns for faceting based on the years with data
  num_cols <- ifelse(length(years_with_data) > 3, 2, 1)
...
#### Overall Count & Percentage of Bighorn Sheep Detections by Age:

The following graph demonstrates the overall detection of Bighorn Sheep by age.

```{r, echo=FALSE, warning = FALSE}

#Combine unknown & unknown/other
sheep_standardize <- sheep_standardize %>%
  mutate(Age_Species1 = ifelse(Age_Species1 == "Unknown/Other", "Unknown", Age_Species1))

#get rid of rows where the age is unknown
sheep_standardize <- sheep_standardize %>%
  filter(!is.na(Age_Species1))


unique_age_names <- unique(sheep_standardize$Age_Species1)
unique_age_names_no_na <- unique_age_names[!is.na(unique_age_names)]

coul <- RColorBrewer::brewer.pal(length(unique_age_names_no_na), "Paired")

counts2 <- sheep_standardize %>%
  group_by(Age_Species1) %>%
  dplyr::summarize(Total = sum(Count_Species1), na.rm=TRUE) %>%
  ungroup()

counts2 <- setNames(counts2$Total, counts2$Age_Species1)
...
interactive_bar <- plot_ly(
  ...
  layout(
    title = "Overall Count of Bighorn Sheep Detection by Age",
    xaxis = list(title = "Sex"),
    yaxis = list(title = "Total Count"),
    barmode = 'group'
  )
...
total_counts_age <- sum(counts2)
percentages <- round(100 * counts2 / total_counts_age, 1)
labels <- paste(names(counts2), "-", percentages, "%", sep="")

# Create an interactive pie chart using plotly
p <- plot_ly(labels = labels, values = counts2, type = 'pie', 
             textinfo = 'label+percent', hoverinfo = 'label+percent+value',
             marker = list(colors = coul)) %>%
  layout(title = 'Percentage of Total Bighorn Sheep Detection by Age')

# Display the plot
p
```

#### Park-wide Annual Detection of Bighorn Sheep by Age
...
# Plotting
pa <- ggplot(plot_df, aes(x = factor(Year), y = percent, fill = Age_Species1)) +
  geom_bar(stat = "identity", position = position_dodge(), width = 0.8) + # Use dodge to place bars side by side
  geom_text(aes(label = paste(percent, "%")), position = position_dodge(width = 0.6), vjust = -0.25, size=3) +
  scale_fill_brewer(palette = "Paired", name = "Age") + # Set a color palette and name the legend as "Age"
  labs(title = "Annual Percentage of Bighorn Sheep by Year and Age (Park-wide)",
       x = "Year", 
       y = "Percentage") +
  theme_minimal() + # Use a minimal theme for a cleaner look
  theme(legend.title = element_text(size = 12), # Adjust legend title size
...
#### Park-wide Annual & Monthly Detection of Bighorn Sheep by Age

``` {r, fig.width = 8, echo = FALSE}

years_with_data <- unique(plot_df_monthly_complete$Year[!is.na(plot_df_monthly_complete$percent)])

# Calculate the number of columns for the facet_wrap based on the number of years with data
num_cols <- ifelse(length(years_with_data) > 3, 2, 1)
...
#### Regional Detection of Bighorn Sheep by Age
...
p <- ggplot(sheep_age_by_region, aes(x = Region, y = percent, fill = Age_Species1)) + 
  ...
  facet_wrap(~Year, ncol = num_cols + 1, scales = "free_y") + 
  scale_fill_brewer(palette = "Paired", name = "Gender") + 
  labs(title = "Annual Percentage of BHS Detections by Age and Region by Yearly Totals",
       x = "Region", 
       y = "Percentage") + 
  theme_minimal() + 
  theme(axis.text.x = element_text(angle = 45, vjust = 0.5, hjust = 0.5),
...
#### Percentage of BHS by Region, Year, Month, & Age Relative to Region Totals
...
### Annual Park-wide Number of Cameras:

```{r cameras, echo=FALSE, message = FALSE}
#Create the Regionsite column if it does not already exist
...
pc <- ggplot(cameras_per_year, aes(x = Year, y = UniqueCameras, fill = RegionSite)) +
  geom_col() +
  theme_minimal() +
  labs(title = "Annual Park-wide Number of Cameras by RegionSite",
       x = "Year",
...
### Unique Cameras per Month by RegionSite and Year

```{r cameramonth, echo=FALSE, message = FALSE}
num_cols <- ifelse(length(unique(photoData$Year)) >= 3, 2, 1)
...
## Number of Camera Nights

Nights are considered detections between 8pm to 4am.
...
pc <- ggplot(cameras_per_year, aes(x = Year, y = UniqueCameras, fill = RegionSite)) +
  geom_col() +
  theme_minimal() +
  labs(title = "Annual Park-wide Number of Night Cameras by RegionSite",
       x = "Year",
       y = "Total Number of Unique Cameras",
       fill = "RegionSite")
...
### Unique Night Cameras per Month by RegionSite and Year
...
The following figures represent the percentage of detection by all species between `r format(min_date, '%m/%d/%Y')` and `r format(max_date, '%m/%d/%Y')`.
...
```{r, echo=FALSE}
# Park-wide proportions - all time
species1_proportions <- table(photoData$Species1) / nrow(photoData)
```

### Park-wide Percentage of Detections by Species (All Years):

<<<<<<< HEAD
```{r, echo = FALSE}
=======
```{r, figheight=20}
>>>>>>> c1bc23f5a7b4cad15fba736ba4deeb7b558670f8
...
<<<<<<< HEAD
p <- p %>% layout(title = 'Park-wide Proportions of Species')
=======
p <- p %>% layout(title = 'Proportions of Species All Time')
>>>>>>> c1bc23f5a7b4cad15fba736ba4deeb7b558670f8
...
<<<<<<< HEAD
### Park-wide Yearly Percentage of detections by species:
...
for (y in df_list) {
  ...
  year <- y$Year[1]
  labels <- names(species_proportions)  # Labels for the pie chart
  values <- species_proportions  # Values for the pie chart
<<<<<<< HEAD
  p <- plot_ly(labels = labels, values = values, type = 'pie', textinfo = 'label+percent',
               marker = list(colors = rainbow(length(values)))) %>%
       layout(title = paste('Proportions of Species in', year))
  # Display the plot
  # Add the plot to the list
  pie_plots[[year]] <- p
}
# Display all plots together
htmltools::tagList(pie_plots)
```

### Monthly Species Sightings by Year
=======
...
tagList(plots_list)
```
### Parkwide Percentage of detections by species (Monthly):
```{r months, echo=TRUE}
library(plotly)
library(htmltools)
...
for (m in sort(unique(photoData$month))) {
  monthData <- photoData[photoData$month == m, ]
  species_proportions <- table(monthData$Species1) / nrow(monthData)
  monthName <- monthData$monthName[1]
  labels <- names(species_proportions)  # Labels for the pie chart
  values <- species_proportions  # Values for the pie chart
  ...
  p <- plot_ly(labels = labels, values = values, type = 'pie', textinfo = 'label+percent',
               marker = list(colors = rainbow(length(values)))) %>%
       layout(title = paste('Proportions of Species in', monthName))
...
The set of charts below show the count of detections of species in a month of a given year
>>>>>>> c1bc23f5a7b4cad15fba736ba4deeb7b558670f8
...
daily_sightings <- species %>%
  mutate(Date = as.Date(DateTime),     # Convert datetime to date
         Year = format(Date, "%Y"),    # Extract year
         Month = format(Date, "%m"),
         Count_Species1 = ifelse(is.na(Count_Species1), 0, as.numeric(Count_Species1))) %>%
...
### Regional Species Sightings (Monthly/Annual):
