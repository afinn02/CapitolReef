...
#ALTERNATE WAY TO DO WHAT WAS IN THE PREVIOUS CODE BLOCK
...
##Parkwide Percentage of detections by species (Monthly/Annual):
```{r months}
for (y in df_list) {
  year <- y$Year[1]
  print(paste("Species proportions for the months of the year ", year))
  for (m in sort(unique(y$month))) {
    month_df <- y[y$month == m, ]
    m_name = month_df$monthName[1]
    species_proportions <- table(month_df$Species1) / nrow(month_df)
    # Create a pie chart
    pie(species_proportions, labels = NA, main = paste("Proportions of Species", m_name, " ", year), col = rainbow(length(species_proportions)))
...
for (m in unique(photoData$monthName)) {
...
```
### Species Detection by Region — Annually:
```{r overall_region_year, echo=FALSE}
for (r in unique(photoData$Region)) {
  # Subset data for the current region
  data_region <- subset(photoData, Region == r)
  for (y in sort(unique(data_region$Year))) {
    year_region = data_region[data_region$Year == y, ]
    # Calculate species proportions for the current region
    species_proportions <- table(year_region$Species1) / nrow(year_region)

    # Create a pie chart
    pie(species_proportions, labels = NA, main = paste0("Proportions of Species ", r , " in ", y), col = rainbow(length(species_proportions)))
...
### Species Detection by Region — Monthly:
```{r overall_region_month, echo=FALSE}
for (r in unique(photoData$Region)) {
  # Subset data for the current region
  data_region <- subset(photoData, Region == r)
  for (m in sort(unique(data_region$month))) {
    month_region = data_region[data_region$month == m, ]
    # Calculate species proportions for the current region
    species_proportions <- table(month_region$Species1) / nrow(month_region)
    monthName = month_region$monthName[1]

    # Create a pie chart
    pie(species_proportions, labels = NA, main = paste0("Proportions of Species ", r , " in ", monthName), col = rainbow(length(species_proportions)))
