...
#THIS FUNCTION EXCLUDES REPEATS
...
p0 <- ggplot(daily_sightings, aes(x = Date, y = Sightings)) +
  geom_line(color = "seagreen4") +  # Set the line color to blue
  geom_point(color = "black") +  # Optionally set the points to the same color
...
gender_sightings <- sheep_standardize %>%
  mutate(Date = as.Date(DateTime)) %>%
  group_by(Date, Sex_Species1) %>%
  summarise(Sightings = n(), .groups = 'drop')

# Create the facet plot
pg <- ggplot(gender_sightings, aes(x = Date, y = Sightings, color = Sex_Species1)) +
...
The following graphs demonstrate what percentage of BHS were detected in a particular region based on the total yearly detections. This is further broken down to the month and the sex.

```{r, echo = FALSE}
yearly_totals <- sheep_standardize %>%
  group_by(Year, Region) %>%
  summarise(TotalYearlyDetections = n(), .groups = 'drop')
